...
_bool mv(_bool remove,char *arg1,char * arg2){
	...
	if (target==NULL||targetDir==NULL||destination==NULL) return _false;
	...
	if (remove && targetDir->pos!=destination->pos) {
		_bool tmp = removeLeaf(targetDir,(desiredName[strlen(desiredName)-1] != '/')?desiredName:target->fileName);//remove the target file from the old dir
		if (!tmp) {
